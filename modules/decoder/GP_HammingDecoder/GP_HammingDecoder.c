...
int parityBitCalc_HammingDecoder(GP_HammingDecoderStruct *GP_HammingDecoder, uint8_t* input, int position)
{
    int count = 0, i, j; 
    i = position - 1;  
 
    // Traverse to store Hamming Code
    while (i < GP_HammingDecoder->N) {
 
      for (j = i; j < i + position; j++) { // j = 1, j <  3 
            // If current bit is 1
            if (input[j] == 1)
                count++;
        }
...
void hammingDecoder(GP_HammingDecoderStruct *GP_HammingDecoder, uint8_t* input, uint8_t* output){
  int error_pos = 0, j = 0, k = 0, i = 0;
  int temp_input[GP_HammingDecoder->N];

  // Assign input array to this temp input  array
  for(i=0; i < GP_HammingDecoder->N; i++){
    temp_input[i] = input[i];
    }
  
    for(i=0; i < GP_HammingDecoder->M; i++){
      int position = (int)pow(2,i);
      int value = parityBitCalc_HammingDecoder(GP_HammingDecoder, input, position);
      if(value != 0)           // If the number of one is not even
	error_pos+=position;
    }
    ...
    if(error_pos == 0){
      ...
      if (input[error_pos-1] == 1) {
	temp_input[error_pos-1] = 0;
      }
      else {
	temp_input[error_pos-1] = 1;
      }
   
    }

 
    printf("After corrected the error bit:\n");
    ...
    // printf("\n+++For testing, look at the following output instead of signal output:\n");
    // for (i = 0; i < GP_HammingDecoder->K; i++) {
    // 	printf("Decoder Output[%d]: %d\n", i, output[i]);
    // }
...
void runGP_HammingDecoder (GP_HammingDecoderStruct *GP_HammingDecoder, signalStruct *signal) {
  ...
  output   = (uint8_t*) incrementByOneSignal(signal,N_output,0,"GP_HammingDecoder");

  // Create a new output vector on end of signal list
  
  // ++++ Testing the code by generating input with one error bit ++++
  // Input message: 0011
  // Correct encoded message:  {1, 0, 0, 0, 0, 1, 1}; 
  // uint8_t input_test[] =  {1, 0, 0, 0, 0, 1, 1};
  // uint8_t output_test[4];
  // hammingDecoder(GP_HammingDecoder, input_test, output_test);

  // For changing the double type input array back to uint8 type
  uint8_t temp_input[7];
  for (i=0;i<GP_HammingDecoder->N;i++) {
    if (input[i] == (double) 1) {
      temp_input[i] = 1;
    }
    ...
    // printf("temp_input[%d]: %d\n", i, temp_input[i]);
    
  }
  
  // Computation engine :
  hammingDecoder(GP_HammingDecoder, temp_input, output);
